...
int CBaseWeaponWorldModel::DrawModel( int flags )
{
    ...
    int ret = BaseClass::DrawModel( flags );
        ...
        if ( !CSLoadout()->IsKnife( this->GetCSWeaponID() ) )
        {
            C_CSPlayer *pPlayer = dynamic_cast<C_CSPlayer*>( GetOwner() );
            int weaponID = this->GetCSWeaponID();
            int iPaintKit = g_pCSLoadout->GetWeaponSkinForPlayerInt( pPlayer, weaponID );
            IMaterial* pMat = g_SkinDatabase.GetSkinMaterial( iPaintKit );
            if ( pMat )
            {
                this->SetMaterialOverride( pMat );
                BaseClass::DrawModel(flags);
                this->SetMaterialOverride(nullptr);
            }
            else
            {
                return ret;
