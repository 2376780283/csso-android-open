...
// ConVar to control auto-spawning
static ConVar sunlight_auto_entity( "sunlight_auto_entity", "1", 0, "Automatically create sunlight_shadow_control if not present in map" );
...
float CSunlightShadowControl::m_flEnvLightShadowPitch = 0.0f;
QAngle CSunlightShadowControl::m_EnvLightShadowAngles = QAngle(0, 0, 0);
bool CSunlightShadowControl::m_bEnvLightShadowValid = false;
...
// Global pointer to the sunlight shadow control entity
static CSunlightShadowControl *g_pSunlightShadowControl = NULL;
...
void CSunlightShadowControl::UpdateEnvLight()
{
	if ( !m_bEnvLightShadowValid )
	{
		Msg( "**** UpdateEnvLight: env light data not valid yet\n" );
		return;
	}
	...
	Msg( "**** UpdateEnvLight: Updated shadow direction from light_environment (pitch: %.2f, yaw: %.2f)\n", 
		angles.x, angles.y );
}

void CSunlightShadowControl::SetEnvLightShadowPitch( float flPitch )
{
	m_flEnvLightShadowPitch = flPitch;
	m_bEnvLightShadowValid = true;
	
	Msg( "**** SetEnvLightShadowPitch: %.2f\n", flPitch );
	
	if ( g_pSunlightShadowControl )
	{
		g_pSunlightShadowControl->UpdateEnvLight();
	}
}

void CSunlightShadowControl::SetEnvLightShadowAngles( const QAngle &angles )
{
	m_EnvLightShadowAngles = angles;
	m_bEnvLightShadowValid = true;
	
	Msg( "**** SetEnvLightShadowAngles: %.2f %.2f %.2f\n", angles.x, angles.y, angles.z );
...
// Auto-spawn manager class
class CSunlightShadowManager : public CAutoGameSystemPerFrame
{
public:
	CSunlightShadowManager()
	{
	}
	
	virtual ~CSunlightShadowManager()
	{
	}

	virtual void LevelInitPreEntity()
	{
        Msg( "**** LevelInitPreEntity\n" );
	}

	virtual void LevelInitPostEntity()
	{
        Msg( "**** LevelInitPostEntity\n" );
        
		if ( g_pSunlightShadowControl ) 
			return;
				
		// Create the sunlight_shadow_control automatically
		CBaseEntity *entity = dynamic_cast< CBaseEntity * >( CreateEntityByName( "sunlight_shadow_control" ) );
		if ( entity )
		{
			entity->Precache();
			entity->KeyValue( "targetname", "sunlight_shadow_control_auto" );
...
static CSunlightShadowManager g_SunlightShadowManager;

// Console command for debugging
void C_Sunlight_Server_Status( const CCommand& args )
{
	Msg( "Entity exists: %u\n", g_pSunlightShadowControl != NULL );
}

static ConCommand sunlight_server_status("sunlight_server_status", C_Sunlight_Server_Status, "Usage:\n sunlight_server_status\n", 0);